    ...
    with st.expander("💬 Talk to HUG" if lang == "English" else "💬 Bicara dengan HUG"):
        ...
        if submitted and user_input:
            ...
            if any(word in user_input.lower() for word in ["hopeless", "give up", "suicidal", "kill", "end it"]):
                st.warning("💔 That sounds really heavy. Please consider calling a crisis line or reaching out to someone you trust." if lang == "English" else "💔 Itu terdengar sangat berat. Pertimbangkan untuk menghubungi layanan krisis atau seseorang yang kamu percaya.")
                ...
                st.info("💙 It’s okay to feel what you’re feeling. You’re not alone." if lang == "English" else "💙 Tidak apa-apa merasakan apa yang kamu rasakan. Kamu tidak sendiri.")
            else:
                st.success("✅ Thank you for sharing." if lang == "English" else "✅ Terima kasih sudah berbagi.")
...
if st.session_state.bully_mode:
    ...
    st.markdown("Let’s talk it through safely." if lang == "English" else "Mari kita bicarakan dengan aman.")
    with st.form("bully_chat"):
        msg = st.text_area("What’s been going on?" if lang == "English" else "Apa yang terjadi?")
        if st.form_submit_button("Send" if lang == "English" else "Kirim") and msg:
            st.markdown(f"🫂 You said: *{msg}*" if lang == "English" else f"🫂 Kamu bilang: *{msg}*")
            st.success("You’re not overreacting. No one deserves this." if lang == "English" else "Kamu tidak berlebihan. Tidak ada yang pantas diperlakukan seperti itu.")
    st.stop()
